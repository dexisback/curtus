...
/** Same rules as server streak bump after a completed focus session. */
export async function bumpStreak(userId: string, completedAt: Date): Promise<void> {
  ...
  const last = existing.lastActiveDate ? getStudyDayStart(existing.lastActiveDate).getTime() : null;
