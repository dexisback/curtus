...
function splitCorsOrigins(raw: string | undefined): string[] {
  if (!raw) return [];
  return raw
    .split(',')
    .map((s) => stripAuthOrigin(s))
    .filter(Boolean);
}
...
export function getSocketCorsOrigins(): string[] {
  ...
  // Optional explicit override (comma-separated) for deployments.
  for (const o of splitCorsOrigins(process.env.CORS_ORIGINS)) add(o);
